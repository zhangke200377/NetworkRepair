...
linux = cmd("awk -F= '/^NAME/{print $2}' /etc/os-release")
if("Manjaro" not in linux and "Arch" not in linux):
    path = "/usr/local/share/ca-certificates/"
    command = "sudo update-ca-certificates"
else:
    path = "/etc/ca-certificates/trust-source/anchors/"
    command = "sudo trust extract-compat"
if(not os.path.exists(path+"ca.crt")):
    print(
        "⭐⭐⭐下面开始安装CA证书，请在提示Password时输入密码，否则后续会出现错误（密码是隐形的，直接打即可）⭐⭐⭐")
    cmd(r'sudo cp "ca/ca.crt" '.replace("ca/", appPath()+"/ca/")+path+"ca.crt")
    cmd(command)
path = "/etc/hosts"
f = open(path, "r")
rawHosts = f.read()
...
if(not os.path.exists(appPath()+"/HOSTS.txt") or "pixiv" not in rawHosts):
    ...
    cmd(r'sudo mv "host/hosts" '.replace("host/", appPath()+"/host/")+path)
...
if(not os.path.exists(appPath()+"/config/config.txt")):
    ...
    f.close()
    print("网速测试结束")
    cmd('sudo chmod +x '+appPath().replace(" ", "\\ ")+r'/bin/main')
    print("Linux因为系统限制，需要每次输入密码，请下次打开时提示Password：时输入你的密码")
else:
...
try:
    ...
    command = r'sudo '+appPath().replace(" ", "\\ ")+r'/bin/main -c "./ca/ca.pem" -k "./ca/ca.key" -l "'.replace("./ca/",
                                                                                                                 appPath()+"/ca/")+appPath()+'/config/domain.conf"'+ps
    # print(command,1)
    cmd(command)
